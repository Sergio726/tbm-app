...
| **S18** | JARVIS — Asistente IA (multi-proveedor) | 📄 diseño | **Documento de diseño:** [`docs/JARVIS_AI_ASSISTANT.md`](docs/JARVIS_AI_ASSISTANT.md). Convierte el orbe-teaser en asistente IA real, **multi-proveedor** (Anthropic/OpenAI/Gemini/DeepSeek) con **API key + modelo configurables desde el panel admin** (Vault). Contexto TBM (método + equipo DISC + tareas). Fases S18.1 cimientos → S18.6 historial+costos. **No implementado** — pendiente de decisiones abiertas (§11 del doc). |
