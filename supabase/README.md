...
| 34 | `migration_s25_kpi_cascade.sql` | **S25 · KPIs en cascada**: `rocks` suma la meta medible del trimestre (`target_value`/`target_unit`/`target_money`) + tablas **`rock_contributions`** (reparto por responsable) y **`contribution_activities`** (llamadas, propuestas). Los totales son **TRIMESTRALES**: el ritmo mensual se DERIVA, no se guarda — guardar la cuota mensual invitaría a evaluar mes por mes, que es el "retrovisor" que el método critica. RLS con **autogestión**: el dueño edita lo suyo, el Arquitecto todo lo de su empresa; quitar un responsable es solo del Arquitecto. **No toca `kpis` ni `leading_indicators`** (son otros dos conceptos) | ⏳ **pendiente de aplicar** |
